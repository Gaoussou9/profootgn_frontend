...
export default function TopScorers() {
  ...
  const parseResponse = (data) => {
    ...
    const norm = arr.map((it, idx) => {
      ...
      const first = p.first_name ?? it.player_first_name ?? it.first_name ?? "";
      const last = p.last_name ?? it.player_last_name ?? it.last_name ?? "";
  ...
  const fetchData = async () => {
    try {
      const r = await api.get(
        "stats/topscorers/?include_live=1&limit=100"
      );
    ...
    <section className="max-w-3xl mx-auto">
      ...
      <p className="text-xs text-gray-500 mt-2">
        * Classement mis à jour automatiquement
        (inclut les buts en cours si disponibles).
      </p>
    </section>
  );
}