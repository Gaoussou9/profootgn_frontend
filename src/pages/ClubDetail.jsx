...
export default function ClubDetail() {
  ...
  function PlayerGroupMobile({ title, list }) {
    ...
    return (
      <div className="rounded-2xl ring-1 ring-black/10 bg-white overflow-hidden mb-4">
        ...
        {list.map((p, idx) => (
          <PlayerRowMobile p={p} idx={idx} key={p.id ?? idx} groupName={title} />
        ))}
      </div>
    );
  }

  /* ---------- Tableau joueurs DESKTOP/TABLET ---------- */
  const SectionTable = ({ title, list }) =>
    list.length > 0 && (
      <div className="hidden sm:block rounded-2xl ring-1 ring-black/10 overflow-hidden bg-white">
        {/* Barre titre verte */}
  ...
  function StaffMobile() {
    ...
    return (
      <div className="sm:hidden rounded-2xl ring-1 ring-black/10 overflow-hidden bg-white">
  ...
  return (
    <section className="space-y-4">
      ...
      {/* EFFECTIF PAR POSTES - VERSION MOBILE (cartes empilées) */}
      ...
      {/* EFFECTIF PAR POSTES - VERSION DESKTOP/TABLET */}
