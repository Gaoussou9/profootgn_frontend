...
/* ---------- Carte Match (affiche minute serveur) ---------- */
function MatchCard({ m }) {
  ...
  const isBreak = status === "HT" || status === "PAUSED";
  ...
  // Minute affichée = minute commune envoyée par le serveur
  // (MatchSerializer.current_minute)
  let minuteBadge = null;

  if (isLive) {
    const cur = Number(m.current_minute);
    if (Number.isFinite(cur)) {
      ...
      minuteBadge = null; // si pas dispo on laisse juste "LIVE"
    }
  } else if (isBreak) {
    // Mi-temps / Pause -> badge HT
    minuteBadge = "HT";
  } else if (status === "FT" || status === "FINISHED") {
    // Terminé : pas de minute, juste "Terminé"
    minuteBadge = null;
  }
...
function pickDefaultRound({ live = [], upcoming = [], recent = [] }) {
  ...
  const ups = (upcoming || [])
    .map((m) => ({
      m,
      t: Date.parse(m.datetime) || Infinity,
    }))
  ...
  const rec = (recent || [])
    .map((m) => ({
      m,
      t: Date.parse(m.datetime) || 0,
    }))
    .filter((x) => Number.isFinite(x.t))
    .sort((a, b) => b.t - a.t)[0];
  if (rec && matchRoundNum(rec.m) != null)
    return matchRoundNum(rec.m);
  return null;
}
...
export default function Home() {
  ...
  // Premier fetch (live, upcoming, recent, etc.)
  useEffect(() => {
    let stop = false;
    (async () => {
      setLoad(true);
  ...
  // Refresh LIVE toutes les 15s (on récupère encore current_minute du serveur)
  useEffect(() => {
  ...
  // Refresh autres listes moins souvent
  useEffect(() => {
    const id = setInterval(async () => {
      try {
        const [
  ...
  const statusRank = (s) => {
    ...
    if (uu === "LIVE" || uu === "HT" || uu === "PAUSED")
      return 0;
    if (uu === "FT" || uu === "FINISHED") return 1;
    if (uu === "SCHEDULED" || uu === "NOT_STARTED")
      return 2;
    if (uu === "SUSPENDED") return 3;
    if (uu === "POSTPONED") return 4;
    if (uu === "CANCELED" || uu === "CANCELLED")
      return 5;
    return 6;
  };
  const timeMs = (d) => {
  ...
  // Tri final :
  //  - Les LIVE/HT/PAUSED d'abord,
  //    triés par minute serveur descendante.
  //  - Puis FT par plus récent.
  //  - Puis SCHEDULED par plus proche à venir.
  //  - Puis le reste.
  const feedSorted = useMemo(() => {
    return [...feedFiltered].sort((a, b) => {
      ...
      if (ra === 0) {
        // matchs en cours
        const am = Number.isFinite(
          Number(a.current_minute)
        )
          ? Number(a.current_minute)
          : Number(a.minute ?? 0);
        const bm = Number.isFinite(
          Number(b.current_minute)
        )
          ? Number(b.current_minute)
          : Number(b.minute ?? 0);
        return bm - am; // plus avancé en premier
      }

      if (ra === 1) {
        // FT / FINISHED : plus récent en premier
        return (
          timeMs(b.datetime) - timeMs(a.datetime)
        );
      }

      if (ra === 2) {
        // À venir : le plus proche en premier
        return (
          timeMs(a.datetime) - timeMs(b.datetime)
        );
      }
  ...
  if (loading)
    return <p className="px-3">Chargement…</p>;
  if (error)
    return (
      <p className="px-3 text-red-600">
        Erreur : {error}
      </p>
    );
